...
import { PlantIdentificationForm } from '@/components/forms/plant-identification-form'
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      if (data.success) {
        setRequests(
          data.data
            .filter((req: any) => req.type === RequestType.PLANT_IDENTIFICATION)
            .map((req: any) => ({
              id: req.id,
              title: req.title,
              status: req.status,
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Plant Identification</h1>
          <p className="text-gray-600">Identifikasi spesies tanaman</p>
        </div>
        ...
        {showForm ? (
          <div className="mb-12">
            ...
            <PlantIdentificationForm onSuccess={() => setShowForm(false)} />
          </div>
        ) : (
          <div className="mb-12">
            <button
            ...
            >
              + Buat Plant Identification Request
            </button>
          </div>
        )}
